...
class TestCMADGU04():
  ...
  def test_cMADGU04(self):
    ...
    # 2. NO tenga rol de Comprador ya asignado
    ...
    for row in user_rows:
      ...
      if user_email != "admin@sistema.com":
        # Buscar el badge del rol Comprador en cada fila (gris)
        buyer_role_badges = row.find_elements(By.CSS_SELECTOR, ".bg-gray-100.text-gray-800")
        # Si no encuentra el badge gris (que es el de comprador), entonces NO es comprador
        if len(buyer_role_badges) == 0:
    ...
    if target_user_row is not None:
      ...
      print(f"Cambiando rol del usuario: {user_name} (email: {user_email}) a Comprador")
      ...
      WebDriverWait(self.driver, 10).until(
        expected_conditions.presence_of_element_located((By.XPATH, "//div[contains(text(), 'Comprador')]"))
      )
      
      # Hacer clic en "Comprador"
      buyer_option = self.driver.find_element(By.XPATH, "//div[contains(text(), 'Comprador')]")
      buyer_option.click()
      ...
      user_found_with_buyer_role = False
      ...
      for row in updated_user_rows:
        ...
        if len(name_elements) > 0 and name_elements[0].text == user_name:
          # Verificar que ahora tenga el badge de comprador (gris)
          buyer_badges = row.find_elements(By.CSS_SELECTOR, ".bg-gray-100.text-gray-800")
          if len(buyer_badges) > 0:
            user_found_with_buyer_role = True
            print(f"✅ Usuario '{user_name}' ahora tiene rol de Comprador")
            break
      
      assert user_found_with_buyer_role, f"El usuario '{user_name}' debería tener ahora el rol de Comprador"
      
    else:
      pytest.skip("No se encontró ningún usuario válido (que no sea admin@sistema.com y no tenga rol de Comprador) para realizar el test")
