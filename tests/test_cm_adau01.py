  ...
  def test_cMADAU01(self):
    ...
    for row in user_rows:
      try:
        ...
        if user_email == "admin@sistema.com":
          continue
    ...
    # Verificar que el radio button "Permanente" esté seleccionado por defecto
    # Si no, seleccionarlo
    permanent_radio = self.driver.find_element(By.XPATH, "//input[@type='radio'][@name='suspendType'][@value='permanent' or following-sibling::text()[contains(., 'Permanente')]]")
    if not permanent_radio.is_selected():
      # Hacer clic en el label que contiene "Permanente"
      permanent_label = self.driver.find_element(By.XPATH, "//label[contains(text(), 'Permanente')]")
      permanent_label.click()
      print("✅ Suspensión permanente seleccionada")
    else:
      print("✅ Suspensión permanente ya estaba seleccionada por defecto")
    
    # Hacer clic en el botón "Confirmar"
    confirm_button = self.driver.find_element(By.XPATH, "//button[contains(text(), 'Confirmar')]")
    confirm_button.click()
    
    print("✅ Confirmación de suspensión enviada")
      ...
      try:
        ...
        if user_email == target_user:
          ...
          if status_text == "Suspendido":
            user_suspended = True
            print(f"✅ Usuario {target_user} fue suspendido exitosamente")
            break
          else:
    ...
    print("✅ Test completado: Usuario suspendido permanentemente de forma exitosa")
